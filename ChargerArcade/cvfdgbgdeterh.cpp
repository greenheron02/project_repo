...
void scatter(double *numbers, double *group, int num_size, int root, int rank, int numprocs) {
   ...
   int number_elements_per_section = ceil((double)num_size / numprocs);
   int num_left = num_size;
   ...
   if (rank == root) {
      ...
      for (int mpiproc = 0; mpiproc < numprocs; mpiproc++) {
         if (num_left - number_elements_per_section < 0)
            number_elements_per_section = num_left;
         num_left -= number_elements_per_section;
         ...
         else if (number_elements_per_section > 0) { // Modified: Ensure data is sent only if valid
   ...
   // if a non root process just receive the data
      ...
      // Modified: Ensure only valid data is received
      int elements_to_receive = min(number_elements_per_section, max(0, num_size - rank*number_elements_per_section));
      if (elements_to_receive > 0) {
         MPI::COMM_WORLD.Recv(group, elements_to_receive, MPI::DOUBLE,
                              root, MPI_ANY_TAG, status);
      }
   }
}
...
void reduce(int *histogram, int *histogram_local,int histogram_size, 
   ...
   if (rank==root) {
      ...
      for(int mpiproc=0;mpiproc<numprocs;mpiproc++) {
         if (mpiproc!=root) {
            int recv_size = (mpiproc == numprocs - 1) ? histogram_size - (mpiproc * histogram_size / numprocs) : histogram_size / numprocs;
            MPI::COMM_WORLD.Recv(histogram_local, recv_size, MPI::INT, mpiproc, MPI_ANY_TAG, status);
            // Add received histogram to the root's histogram
            for (int i=0; i<recv_size; i++) {
               histogram[i + mpiproc * (histogram_size / numprocs)] += histogram_local[i];
            }
         } 
      }
   }
   ...
   // process' local histogram data to the root MPI process
      ...
      int send_size = (rank == numprocs - 1) ? histogram_size - (rank * histogram_size / numprocs) : histogram_size / numprocs;
      MPI::COMM_WORLD.Send(histogram_local, send_size, MPI::INT, root, tag);
   }
}
