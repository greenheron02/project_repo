...
Notes: This implementation utilizes a minimum set of MPI methods
   ...
   GNU Compiler
      module load mpich
      mpic++ histogram_MPI2.cpp -o histogram_MPI2

To execute:
   
  On GNU compiler
     use run_script histogram_MPI2.sh
...
void scatter(double *numbers, double *group, int num_size, int root, int rank, int numprocs) {
   ...
   int num_elements_per_process = num_size / numprocs; 
   int leftover_elements = num_size % numprocs;
   ...
   if (rank == root) {
      ...
      for (int mpiproc = 0; mpiproc < numprocs; mpiproc++) {
         int num_elements = num_elements_per_process + (mpiproc < leftover_elements);
         ...
         // the next MPI process
            ...
            MPI::COMM_WORLD.Send(&numbers[begin_element], num_elements, MPI::DOUBLE, mpiproc, tag);
         }
         begin_element += num_elements;
      }
   }
  // if a non root process just receive the data
  else {
      int num_elements = num_elements_per_process + (rank < leftover_elements );
	  cout << rank << " gets " << num_elements << ", with  " << num_elements_per_process << " " << leftover_elements << "\n";
      if (num_elements > 0) {
         MPI::COMM_WORLD.Recv(group, num_elements, MPI::DOUBLE, root, MPI_ANY_TAG, status);
      }
   }
}
...
void reduce(int *histogram, int *histogram_local,int histogram_size, 
   ...
   int tag = 123;
   const int num_elements_per_process = histogram_size / numprocs;
   int leftover_elements = histogram_size % numprocs;
   ...
   if (rank==root) {
      ...
      int num_elements = num_elements_per_process + (0 < leftover_elements);
      ...
      for(int mpiproc=0;mpiproc<numprocs;mpiproc++) {
         if (mpiproc!=root) {
			num_elements = num_elements_per_process + (mpiproc < leftover_elements);
		
			if(num_elements > 0)
				MPI::COMM_WORLD.Recv(histogram_local,num_elements,MPI::INT,
   ...
   // process' local histogram data to the root MPI process
		...
		int num_elements = num_elements_per_process + (rank < leftover_elements );
		if(num_elements > 0)
			MPI::COMM_WORLD.Send(histogram_local, num_elements, MPI::INT, root, tag);
   }
}
...
int main(int argc, char *argv[]) {
   ...
   int num_elements_per_process = list_size / numprocs; 
   int leftover_elements = list_size % numprocs;
   int numbers_local_array_sz = num_elements_per_process + (rank < leftover_elements)+1; 
   ...
   const int local_list_size = num_elements_per_process + (rank < leftover_elements ? 1:0);
   cout << rank << " will calc " << local_list_size << "\n: ";
